...
Extracts key information from ICICI credit card statements
"""
...
def parse_icici_statement(text: str) -> Dict[str, Optional[str]]:
    """
    ...
    cycle_patterns = [
        r"Statement\s+(?:Date|Period)[:\s]+(\d{1,2}\s+[A-Za-z]{3}\s+\d{4}\s*[-–to]+\s*\d{1,2}\s+[A-Za-z]{3}\s+\d{4})",
        r"Billing\s+Period[:\s]+(\d{1,2}\s+[A-Za-z]{3}\s+\d{4}\s*[-–to]+\s*\d{1,2}\s+[A-Za-z]{3}\s+\d{4})",
    ...
    due_date_patterns = [
        r"Payment\s+Due\s+(?:Date|By)[:\s]+(\d{1,2}\s+[A-Za-z]{3}\s+\d{4})",
...
def validate_icici_statement(text: str) -> bool:
    """
    Validate if the PDF is an ICICI Bank statement
    
    Args:
        text: Extracted text from PDF
        
    Returns:
        True if ICICI statement, False otherwise
    """
    icici_keywords = [
        "ICICI Bank",
        "ICICI BANK",
    ...
    return any(keyword.lower() in text_lower for keyword in icici_keywords)
